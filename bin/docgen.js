...
const { argv } = require("yargs/yargs")(require("yargs/helpers").hideBin(process.argv))
	...
	.command("$0", `Generate ${require("../terminal-link")("Docma", "https://github.com/onury/docma")} documentation from TypeScript code annotated with JSDoc.`)
	.option("src", {
		alias: "s",
	...
	.demandOption(["src", "out"], "Please specify a source and output directory.")
	.option("temp", {
...
docmaConfig = fs.existsSync(docmaConfig) ? require(docmaConfig) : null;
...
inputFiles.forEach((file) => {
	...
	const filename = `${path.parse(file).name}.ts`;
	const transpiledJS = transpile(fileContent, filename);
...
if (!argv.noDocs) {
	...
	(async () => {
		...
		const assetPaths = argv.assets.map((a) => normalizePathArgument(a));
